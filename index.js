...
app.use(session({
    secret: "mysecret", resave: false, saveUninitialized: true, cookie: {
        httpOnly: true, maxAge: Date.now() + 1000 * 60 * 60 * 2, sameSite: process.env.NODE_ENV === "production" ? 'none' : 'lax', // must be 'none' to enable cross-site delivery
...
try{
    await user.register(temp, req.body.user.password)}catch(e){
console.log(e)
